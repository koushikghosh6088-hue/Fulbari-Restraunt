...
export default function MenuPage() {
    ...
    const categories = ["All", ...Array.from(new Set(filteredByMenuType.map(item => `${item.category || "General"} ${item.isVeg ? '(Veg)' : '(Non-Veg)'}`))).sort()];
    ...
    const filteredItems = filteredByMenuType.filter((item) => {
        ...
        const itemCategoryLabel = `${item.category || "General"} ${item.isVeg ? '(Veg)' : '(Non-Veg)'}`;
                                            ...
                                            >
                                                ...
                                                <div className="p-4 flex-grow flex flex-col">
                                                    ...
                                                    <div className="mt-auto pt-4 border-t border-border/40">
                                                        {item.variant_prices && typeof item.variant_prices === 'object' && Object.entries(item.variant_prices).map(([k, v]: any) => (
