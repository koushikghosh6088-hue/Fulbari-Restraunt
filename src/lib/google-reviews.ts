    ...
    try {
        ...
        if (data.status === "OK" && data.result && data.result.reviews) {
            ...
            const apiReviews = data.result.reviews.map((r: any, idx: number) => {
                ...
                const dateLabel = getTimeAgo(r.time);
                ...
                return {
                    ...
                    // Missing foodImg intentionally to let UI handle fallback
                };
            });

            // If the user expects MORE than 5, we can append standard fallback ones
            // but for now let's combine Google top 5 + some unique ones from fallback
            // to keep the layout full if they want pagination
            const apiNames = new Set(apiReviews.map((ar: any) => ar.name));
            const filteredFallbacks = FALLBACK_REVIEWS.filter(fr => !apiNames.has(fr.name));
